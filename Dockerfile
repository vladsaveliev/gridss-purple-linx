...
RUN apt-get update
RUN DEBIAN_FRONTEND=noninteractive apt-get install -y openjdk-8-jre-headless samtools sambamba bwa build-essential make r-base libssl-dev libcurl4-openssl-dev libxml2-dev circos pkg-config libgd-dev
...
COPY external/hmftools/count-bam-lines/target/count-bam-lines-1.7-jar-with-dependencies.jar /jar/
...
COPY external/gridss/example/gridss.sh /scripts/
COPY external/scripts/gridss/*.R /scripts/
